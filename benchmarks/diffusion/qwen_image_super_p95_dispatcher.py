...
QWEN_IMAGE_SCHEDULING_MODE_RELEASE_CALENDAR_TAIL_PACK_BACKFILL = "release_calendar_tail_pack_backfill"
...
def _set_default_backend_env(args: argparse.Namespace) -> None:
    configured_keys = {item.split("=", 1)[0].strip() for item in args.backend_env if "=" in item}
...
def apply_qwen_image_scheduling_mode(args: argparse.Namespace) -> argparse.Namespace:
    if args.qwen_image_scheduling_mode == QWEN_IMAGE_SCHEDULING_MODE_RELEASE_CALENDAR_TAIL_PACK_BACKFILL:
