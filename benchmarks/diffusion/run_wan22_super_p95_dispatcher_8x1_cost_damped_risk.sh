...
TAIL_IDLE_BACKFILL="${TAIL_IDLE_BACKFILL:-0}"
...
LONG_REQUEST_RATIO="${LONG_REQUEST_RATIO:-1.5}"
...
TAIL_IDLE_BACKFILL_ARGS=()
if [[ "${TAIL_IDLE_BACKFILL}" == "1" ]]; then
  TAIL_IDLE_BACKFILL_ARGS+=(--tail-idle-backfill)
fi
...
python3 benchmarks/diffusion/wan22_super_p95_dispatcher.py \
  ...
  --long-request-ratio "${LONG_REQUEST_RATIO}" \
