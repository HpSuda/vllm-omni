...
def apply_release_calendar_tail_pack_backfill_defaults(
    ...
    backend_scheduler = (getattr(args, "backend_scheduler", None) or "").strip()
    if backend_scheduler and backend_scheduler != "super_p95_step":
        raise ValueError(
            "release_calendar_tail_pack_backfill requires --backend-scheduler super_p95_step; "
            f"got {backend_scheduler!r}"
        )
...
class ManagedBackendLauncher:
    ...
    def _start_one(self, spec: ManagedBackendSpec) -> ManagedBackendProcess:
        ...
        # The backend scheduler consumes the hardware profile from its
        # environment; no hardware-profile serve CLI flag is required.
...
class SuperP95Dispatcher:
    ...
    async def _maybe_release_video_load_from_get(self, path: str, response: httpx.Response) -> None:
        ...
        try:
            ...
            return
        # The video API returns the terminal job object with an HTTP error
        # status when generation fails. Trust only a matching job identity,
        # not the HTTP code: a generic 404 or proxy error must not free a slot.
    ...
    async def health(self) -> JSONResponse:
        ...
        return JSONResponse(
            ...
            content={
                ...
                "running_tail_preemptible_for_normal": (self.running_tail_preemptible_for_normal),
                "service_time_estimator": self.service_time_estimator_name,
    ...
    def _build_forward_headers(incoming_headers: dict[str, str], decision: DispatchDecision) -> dict[str, str]:
        blocked = {
            "host",
            "content-length",
            HEADER_SUPER_P95_ESTIMATED_SERVICE_S.lower(),
            HEADER_SUPER_P95_SACRIFICIAL.lower(),
        }
        headers = {key: value for key, value in incoming_headers.items() if key.lower() not in blocked}
        if decision.request_id and not any(key.lower() == "x-request-id" for key in headers):
...
def _estimate_service_s_from_values(
    *,
    ...
    request = OmniDiffusionRequest(prompt="super_p95", sampling_params=sampling_params, request_id="super_p95")
...
def _parse_usp_degree(backend_args: list[str]) -> int:
    ...
    for idx, token in enumerate(backend_args):
        ...
        if option not in {"--usp", "--ulysses-degree"}:
            continue
        if not separator:
            value = backend_args[idx + 1] if idx + 1 < len(backend_args) else ""
        try:
            degree = int(value)
        except ValueError as exc:
            raise ValueError(f"{option} requires a positive integer, got {value!r}") from exc
        if degree < 1:
            raise ValueError(f"{option} requires a positive integer, got {value!r}")
    return degree
...
def _parse_device_ids(device_ids: str | None, num_servers: int, usp_degree: int) -> list[str]:
    if device_ids is None:
        return [
            ",".join(str(index) for index in range(server * usp_degree, (server + 1) * usp_degree))
...
def build_dispatcher_from_args(
    ...
    *,
    ...
    if manual_urls:
        ...
        # The parser supplies --omni and callers may repeat that boolean flag.
        # Preserve every other token: equal values can belong to different
        # options (e.g. --usp 1 --cfg-parallel-size 1), and repeatable options
        # may intentionally occur more than once.
        deduped_backend_args: list[str] = []
        for token in backend_args:
            if token == "--omni" and token in deduped_backend_args:
                continue
