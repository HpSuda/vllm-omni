...
def test_managed_backend_arguments_preserve_repeated_values_and_options() -> None:
    args = dispatcher_module.build_arg_parser().parse_args(
        [
            "--model",
            "local-model",
            "--num-servers",
            ...
            "--backend-args=--omni",
            "--backend-args=--usp 1",
            "--backend-args=--cfg-parallel-size 1",
            "--backend-args=--lora-modules first=/models/first",
            "--backend-args=--lora-modules second=/models/second",
        ]
    )

    dispatcher = dispatcher_module.build_dispatcher_from_args(args)

    assert dispatcher._backend_launcher is not None
    assert dispatcher._backend_launcher.backend_args == [
        "--omni",
        "--usp",
        "1",
        "--cfg-parallel-size",
...
@pytest.mark.parametrize("backend_arg", ["--usp 4", "--usp=4", "--ulysses-degree 4", "--ulysses-degree=4"])
def test_managed_backend_auto_devices_match_usp_groups(backend_arg: str) -> None:
    args = dispatcher_module.build_arg_parser().parse_args(
        ["--model", "local-model", "--num-servers", "2", f"--backend-args={backend_arg}"]
    )

    launcher = dispatcher_module.build_dispatcher_from_args(args)._backend_launcher

    assert launcher is not None
    assert [spec.device_id for spec in launcher.specs] == ["0,1,2,3", "4,5,6,7"]


@pytest.mark.parametrize("arguments", [["--usp"], ["--usp", "zero"], ["--usp=0"], ["--ulysses-degree=-1"]])
def test_managed_backend_rejects_invalid_usp(arguments: list[str]) -> None:
    with pytest.raises(ValueError, match="requires a positive integer"):
        dispatcher_module._parse_usp_degree(arguments)


def test_service_estimation_constructs_current_single_request(monkeypatch) -> None:
    captured = {}

    def estimate(request, *, hardware_profile):
        captured["request"] = request
        captured["hardware_profile"] = hardware_profile
        return 12.5

    monkeypatch.setattr(dispatcher_module, "estimate_service_time_s", estimate)
    ...
    assert isinstance(request, OmniDiffusionRequest)
    assert request.prompt == "super_p95"
    assert request.request_id == "super_p95"
    assert request.sampling_params.width == 512
    assert request.sampling_params.height == 512
    assert request.sampling_params.num_inference_steps == 20
    assert captured["hardware_profile"] == "910B3"
    assert actual == 12.5


def test_forward_headers_propagate_request_identity_and_replace_routing_metadata() -> None:
    decision = dispatcher_module.DispatchDecision(
        backend_index=0,
        estimated_service_s=12.5,
        is_sacrificial=False,
        arrival_counter=1,
        credits_before=0,
        credits_after=0,
        quota_added=0,
        global_max_service_s=12.5,
        request_id="caller-request",
    )

    headers = SuperP95Dispatcher._build_forward_headers(
        {"x-super-p95-sacrificial": "1", "x-super-p95-estimated-service-s": "999"},
        decision,
    )

    assert headers["X-Request-Id"] == "caller-request"
    assert headers[dispatcher_module.HEADER_SUPER_P95_ESTIMATED_SERVICE_S] == "12.500000"
    assert not any(key.lower() == "x-super-p95-sacrificial" for key in headers)

    existing = SuperP95Dispatcher._build_forward_headers({"x-request-id": "existing-header-id"}, decision)
    assert existing["x-request-id"] == "existing-header-id"
    assert sum(key.lower() == "x-request-id" for key in existing) == 1


@pytest.mark.parametrize(
    ("http_status", "payload", "released"),
    [
        (200, {"id": "video-1", "status": "completed"}, True),
        (400, {"id": "video-1", "status": "failed", "error": "invalid input"}, True),
        (500, {"id": "video-1", "status": "failed", "error": "worker failure"}, True),
        (409, {"id": "video-1", "status": "cancelled"}, True),
        (200, {"id": "video-1", "status": "in_progress"}, False),
        (404, {"detail": "Not found"}, False),
        (500, {"status": "failed"}, False),
        (500, {"id": "different-video", "status": "failed"}, False),
        (200, {"id": "different-video", "status": "completed"}, False),
        (500, ["invalid", "job", "response"], False),
    ],
)
def test_video_terminal_feedback_releases_only_the_matching_job(http_status, payload, released) -> None:
    async def run() -> None:
        ...
        decision = await dispatcher._choose_backend(
            "/v1/videos",
            {"request_id": "request-1", "size": "854x480", "num_frames": 80, "num_inference_steps": 3},
        )
        assert dispatcher._remember_video_backend(
            "/v1/videos",
            dispatcher_module.httpx.Response(200, json={"id": "video-1", "status": "queued"}),
            decision,
            request_id="request-1",
            request_received_at_s=dispatcher_module.time.perf_counter(),
        )

        await dispatcher._maybe_release_video_load_from_get(
            "/v1/videos/video-1", dispatcher_module.httpx.Response(http_status, json=payload)
        )

        assert dispatcher.backends[0].inflight_normal_requests == (0 if released else 1)
        assert ("video-1" not in dispatcher._video_decision_by_id) == released
        if released:
            assert dispatcher.backends[0].normal_load_s == 0.0
        if not isinstance(payload, dict) or payload.get("status") != "completed" or not released:
            assert not dispatcher._completed_latency_history_s

    asyncio.run(run())


def test_failed_video_feedback_unblocks_waiting_normal() -> None:
    async def run() -> None:
        dispatcher = SuperP95Dispatcher(
            backend_urls=["http://backend-0"],
            backend_hardware_profiles=["910B3"],
        ...
        body = {"size": "854x480", "num_frames": 80, "num_inference_steps": 3}
        first = await dispatcher._choose_backend("/v1/videos", {**body, "request_id": "first"})
        dispatcher._remember_video_backend(
            "/v1/videos",
            dispatcher_module.httpx.Response(200, json={"id": "video-1"}),
            first,
            request_id="first",
            request_received_at_s=dispatcher_module.time.perf_counter(),
        )
        waiting = asyncio.create_task(dispatcher._choose_backend("/v1/videos", {**body, "request_id": "second"}))
        await asyncio.sleep(0)
        assert not waiting.done()

        await dispatcher._maybe_release_video_load_from_get(
            "/v1/videos/video-1",
            dispatcher_module.httpx.Response(500, json={"id": "video-1", "status": "failed"}),
        )
        second = await asyncio.wait_for(waiting, timeout=1.0)

        assert second.request_id == "second"
        assert not dispatcher._pending_normal_dispatches
        assert not dispatcher._completed_latency_history_s
        await dispatcher._mark_failed_response(second, 0.0)
...
def test_release_calendar_beam_treats_preemptible_tail_as_available(
    ...
    async def _run():
        ...
        queued_tasks = [task for task in tasks if task is not running_on_tail_backend_task]
        await dispatcher._mark_failed_response(
            incumbent,
            elapsed_s=1.0,
        )
