...
from benchmarks.diffusion.qwen_image_super_p95_dispatcher import (
    ...
    apply_qwen_image_scheduling_mode,
...
def test_release_calendar_tail_pack_backfill_mode_applies_qwen_defaults() -> None:
    ...
    assert "VLLM_OMNI_ENABLE_DIFFUSION_SERVER_SCHEDULING" not in backend_env
    assert "VLLM_OMNI_ENABLE_DIFFUSION_PREEMPTION" not in backend_env


def test_qwen_managed_cli_selects_scheduler_without_legacy_environment_flags() -> None:
    args = parse_args(
        [
            "--model",
            "Qwen/Qwen-Image",
    ...
    launcher = build_qwen_image_dispatcher(args)._backend_launcher

    assert launcher is not None
    assert launcher.backend_scheduler == "super_p95_step"
    assert launcher.backend_args == ["--omni", "--vae-use-slicing", "--vae-use-tiling"]
    assert "VLLM_OMNI_ENABLE_DIFFUSION_SERVER_SCHEDULING" not in launcher.backend_env
    assert "VLLM_OMNI_ENABLE_DIFFUSION_PREEMPTION" not in launcher.backend_env


@pytest.mark.parametrize("scheduler", [None, "", "super_p95_step"])
def test_qwen_named_mode_requires_preemptible_backend(scheduler: str | None) -> None:
    args = parse_args([])
    args.qwen_image_scheduling_mode = QWEN_IMAGE_SCHEDULING_MODE_RELEASE_CALENDAR_TAIL_PACK_BACKFILL
    args.backend_scheduler = scheduler

    configured = apply_qwen_image_scheduling_mode(args)

    assert configured.backend_scheduler == "super_p95_step"


def test_qwen_named_mode_rejects_incompatible_explicit_backend() -> None:
    with pytest.raises(ValueError, match="requires --backend-scheduler super_p95_step"):
        parse_args(
            [
                "--qwen-image-scheduling-mode",
                QWEN_IMAGE_SCHEDULING_MODE_RELEASE_CALENDAR_TAIL_PACK_BACKFILL,
...
def test_qwen_custom_mode_preserves_backend_choice() -> None:
    args = parse_args(["--backend-scheduler", "step_baseline"])

    assert args.backend_scheduler == "step_baseline"
