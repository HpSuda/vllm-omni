...
def test_wan22_named_mode_requires_preemptible_backend(scheduler: str | None) -> None:
    args = parse_args([])
    args.wan22_scheduling_mode = WAN22_SCHEDULING_MODE_RELEASE_CALENDAR_TAIL_PACK_BACKFILL
    args.backend_scheduler = scheduler

    configured = apply_wan22_scheduling_mode(args)

    assert configured.backend_scheduler == "super_p95_step"


def test_wan22_named_mode_rejects_incompatible_explicit_backend() -> None:
...
def test_wan22_custom_mode_preserves_backend_choice() -> None:
