...
EXTRA_ARG_SUPER_P95_TRACE_REQUEST_ID = "_super_p95_trace_request_id"
...
def apply_super_p95_request_headers(extra_args: dict[str, Any], headers: Mapping[str, str] | None) -> None:
    if not headers:
        return
    # Plain dicts (unlike HTTP header classes) are case-sensitive.
    headers = {key.lower(): value for key, value in headers.items()}
    sacrificial = _parse_bool_header(headers.get(HEADER_SUPER_P95_SACRIFICIAL.lower()))
    if sacrificial is not None:
        extra_args[EXTRA_ARG_SUPER_P95_SACRIFICIAL] = sacrificial
    estimated_service_s = _parse_float_header(headers.get(HEADER_SUPER_P95_ESTIMATED_SERVICE_S.lower()))
    ...
    if trace_request_id:
        extra_args[EXTRA_ARG_SUPER_P95_TRACE_REQUEST_ID] = trace_request_id


def apply_super_p95_sampling_headers(sampling_params: Any, headers: Mapping[str, str] | None) -> None:
    """Attach scheduling metadata after model-specific request normalization.

    Copy the mapping because sampling defaults may be shared between requests.
    Request IDs are diagnostic only and never participate in priority decisions.
    """
    extra_args = dict(sampling_params.extra_args or {})
    apply_super_p95_request_headers(extra_args, headers)
...
def parse_super_p95_load_headers(headers: Mapping[str, str] | None) -> SuperP95LoadSnapshot | None:
    ...
    headers = {key.lower(): value for key, value in headers.items()}
    normal_load_s = _parse_float_header(headers.get(HEADER_SUPER_P95_NORMAL_LOAD_S.lower()))
    sacrificial_load_s = _parse_float_header(headers.get(HEADER_SUPER_P95_SACRIFICIAL_LOAD_S.lower()))
...
def get_super_p95_request_metadata(extra_args: Mapping[str, Any] | None) -> tuple[bool, float | None]:
    if not extra_args:
        return False, None
    value = extra_args.get(EXTRA_ARG_SUPER_P95_SACRIFICIAL, False)
    sacrificial = value if isinstance(value, bool) else _parse_bool_header(str(value)) is True
    estimated_service_s = extra_args.get(EXTRA_ARG_SUPER_P95_ESTIMATED_SERVICE_S)
    if isinstance(estimated_service_s, (int, float)) and not isinstance(estimated_service_s, bool):
        return sacrificial, _parse_float_header(str(estimated_service_s))
